...
namespace Pawra.BLL.Service
{
    public class ClinicVaccineService : IClinicVaccineService
    {
        ...
        {
            var clinicVaccines = await _context.ClinicVaccines.AsNoTracking().ToListAsync();
            return _mapper.Map<IEnumerable<ClinicVaccineDto>>(clinicVaccines);
        }

        public async Task<ClinicVaccineDto> GetByIdAsync(Guid id)
        {
            var clinicVaccine = await _context.ClinicVaccines.AsNoTracking().FirstOrDefaultAsync(cv => cv.Id == id);
            if (clinicVaccine == null) throw new NotFoundException($"Không tìm thấy vaccine phòng khám với ID: {id}");
