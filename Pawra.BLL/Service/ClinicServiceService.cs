...
namespace Pawra.BLL.Service
{
    public class ClinicServiceService : IClinicServiceService
    {
        ...
        {
            var clinicServices = await _context.ClinicServices.AsNoTracking().ToListAsync();
            return _mapper.Map<IEnumerable<ClinicServiceDto>>(clinicServices);
        }

        public async Task<ClinicServiceDto> GetByIdAsync(Guid id)
        {
            var clinicService = await _context.ClinicServices.AsNoTracking().FirstOrDefaultAsync(cs => cs.Id == id);
            if (clinicService == null) throw new NotFoundException($"Không tìm thấy dịch vụ phòng khám với ID: {id}");
