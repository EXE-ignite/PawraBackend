...
namespace Pawra.BLL.Service
{
    public class VaccinationRecordService : IVaccinationRecordService
    {
        ...
        {
            var records = await _context.VaccinationRecords.AsNoTracking().ToListAsync();
            return _mapper.Map<IEnumerable<VaccinationRecordDto>>(records);
        }

        public async Task<VaccinationRecordDto> GetByIdAsync(Guid id)
        {
            var record = await _context.VaccinationRecords.AsNoTracking().FirstOrDefaultAsync(v => v.Id == id);
            if (record == null) throw new NotFoundException($"Không tìm thấy hồ sơ tiêm chủng với ID: {id}");
